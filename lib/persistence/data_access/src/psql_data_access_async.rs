use diesel_async::{
    result::Error, AsyncConnection, ConnectionResult, ExpressionMethods, AsyncPgConnection, QueryDsl,
...
impl PsqlDataAccess {
    ...
    ///
    /// Requires a pg_connection, an in_file_meta as parameters and returns a Result<models::file_meta::FileMeta, async_diesel::result::Error>
    async fn insert_file_meta(
        &self,
        pg_connection: &mut PgConnection,
        in_file_meta: &models::file_meta::FileMeta,
    ) -> Result<models::file_meta::FileMeta, async_diesel::result::Error> {
        use models::schema::file_meta;

        let result = async_diesel::insert_into(file_meta::table)
            .values(in_file_meta)
            .returning(models::file_meta::FileMeta::as_returning())
            .get_result(pg_connection)
            .expect("Error saving file meta").await?;
    ...
    ///
    /// Requires a pg_connection, an in_container_meta as parameters and returns a Result<models::container_meta::ContainerMeta, async_diesel::result::Error>
    async fn insert_container_meta(
        &self,
        pg_connection: &mut PgConnection,
        in_container_meta: &models::container_meta::ContainerMeta,
    ) -> Result<models::container_meta::ContainerMeta, async_diesel::result::Error> {
        use models::schema::container_meta;

        let result = async_diesel::insert_into(container_meta::table)
    ...
    ///
    /// Requires a pg_connection, an in_file_meta_id as parameters and returns a Result<models::file_meta::FileMeta, async_diesel::result::Error>
    async fn get_file_meta_by_id(
        ...
        let result = file_meta
            ...
            .first::<models::file_meta::FileMeta>(pg_connection).await?;
    ...
    ///
    /// Requires a pg_connection, an in_container_meta_id as parameters and returns a Result<models::container_meta::ContainerMeta, async_diesel::result::Error>
    async fn get_container_meta_by_id(
        ...
        let result = container_meta
            ...
            .first::<models::container_meta::ContainerMeta>(pg_connection).await?;
    ...
    ///
    /// Requires a pg_connection, a file_meta_id, an in_file_meta_id as parameters and returns a Result<models::file_meta::FileMeta, async_diesel::result::Error>
    async fn update_file_meta_by_id(
        ...
        let result = async_diesel::update(file_meta.filter(id.eq(file_meta_id)))
            .set((
            ...
            .returning(models::file_meta::FileMeta::as_returning())
            .get_result(pg_connection).await?;
    ...
    ///
    /// Requires a pg_connection, a container_meta_id, an in_container_meta_id as parameters and returns a Result<models::container_meta::ContainerMeta, async_diesel::result::Error>
    async fn update_container_meta_by_id(
        ...
        let result = async_diesel::update(container_meta.filter(id.eq(container_meta_id)))
            .set((
    ...
    async fn delete_file_meta_by_id(
        ...
        pg_connection: &mut PgConnection,
        file_meta_id: &Uuid,
    ) -> Result<(), async_diesel::result::Error> {
        use models::schema::file_meta::dsl::*;

        let rows_deleted =
            async_diesel::delete(file_meta.filter(id.eq(file_meta_id))).execute(pg_connection).await?;
    ...
    async fn delete_container_meta_by_id(
        ...
        let rows_deleted = async_diesel::delete(container_meta.filter(id.eq(container_meta_id)))
...
#[cfg(test)]
mod tests {
    use async_diesel::Identifiable;

    use super::*;

    // In order to run the test execute: `RUST_LOG=info cargo test`
    #[tokio::test]
    async fn test_psql_data_access_methods_for_file_meta() -> Result<(), async_diesel::result::Error> {
        env_logger::init();
        ...
        let mut pg_connection = AsyncPgConnection::establish(&database_url)
        ...
        // [C]reate
        let mut result = psql_data_access.insert_file_meta(&mut pg_connection, &video_file_meta).await?;
        assert!(result.is_ok());
        result = psql_data_access.insert_file_meta(&mut pg_connection, &audio_file_meta).await?;
        assert!(result.is_ok());

        // [R]ead
        result = psql_data_access.get_file_meta_by_id(&mut pg_connection, &video_file_meta.id).await?;
        assert!(result.is_ok());
        ...
        result = psql_data_access.get_file_meta_by_id(&mut pg_connection, &audio_file_meta.id).await?;
        assert!(result.is_ok());
        ...
        let mut delete_result =
            psql_data_access.delete_file_meta_by_id(&mut pg_connection, &video_file_meta.id.await?);
        assert!(result.is_ok());

        delete_result =
            psql_data_access.delete_file_meta_by_id(&mut pg_connection, &audio_file_meta.id.await?);
        assert!(result.is_ok());
